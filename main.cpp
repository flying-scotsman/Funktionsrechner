...
int main(int argc, char** argv)
{
    if(argc > 1)
	{
        ...
        if(!str.is_open())
			cout << "Fehler: Datei konnte nicht gefunden oder geoeffnet werden." << endl;
        else
			evaluate(str);
	...
	{
		cout << "Fehler: Kein Dateiname wurde eingegeben." << endl;		
	}
...
void evaluate(ifstream& str_)
{
    ...
    while(str_.get(c))
	{
        ...
        else if(c == ')')
		{
            ...
            else if(!vals.empty() && !ops.empty() && bcount == 0)
				performOperation(vals, ops);
        }
		...
		{ 
            ...
            else
			{
                ...
                if(!isdigit(c))
				{
					...
					num += to_string(c-48); // -48 wegen UTF-8 Kodierung
            }
        }
    }
